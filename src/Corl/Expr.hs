...
data Expr a
  ...
  | Ap (Expr a) (Expr a) -- Applications
  | Let Rec (NonEmpty (a, Expr a)) (Expr a) -- Let(rec) expressions
  | Case (Expr a) (NonEmpty (Int, [a], Expr a)) -- Case expression
  | Lam [a] (Expr a) -- Lambda abstractions
  | Binary BinOp (Expr a) (Expr a) -- Binary operation
  | Unary UnOp (Expr a) -- Unary operation
